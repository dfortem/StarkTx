...
def decode_abi(raw_abi: dict) -> Dict[str, dict]:
        ...
        for _parameter in _parameters:
            ...
            if _parameter["type"][0] == "(" and _parameter["type"][-1] == ")":
                ...
                for _tuple_item in _parameter["type"][1:-1].split(","):
                    _parameter["tuple_members"] += _flatten_parameters(
                        [dict(name="", type=_tuple_item)], _structures
                    )
                ...
                _parameter["struct_members"] = _flatten_parameters(
                    _structures[_parameter["struct_name"]], _structures
                )
                _parameter["type"] = "struct"
            elif (
                _parameter["type"][-1:] == "*"
                and _parameter["type"][:-1] in _structures
            ):
    ...
    for element in raw_abi:
        ...
        if element["type"] == "constructor":
            ...
            selector = (
                get_selector_from_name(element["name"])
                if element["name"] != "__default__"
                else element["name"]
            )
            inputs = _flatten_parameters(element["inputs"], structures)
            ...
            selector = (
                get_selector_from_name(element["name"])
                if element["name"] != "__l1_default__"
                else element["name"]
            )
            inputs = _flatten_parameters(element["inputs"], structures)
            outputs = _flatten_parameters(element["outputs"], structures)
