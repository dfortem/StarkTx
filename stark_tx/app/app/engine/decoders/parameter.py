...
def decode_parameters(parameters, parameters_abi):
    ...
    while parameters_index < len(parameters):
        ...
        if parameter_type == 'struct':
            ...
            value = decode_atomic_parameter(parameters[parameters_index], parameter_type)
            if (
    ...
    if (
        ...
        selector = hex(decoded_parameters[1]["value"]) \
        ...
        if semantics:
            function_abi = (
                semantics["abi"]["functions"][selector] if selector in semantics["abi"]["functions"] else None
            )
            if function_abi:
