...
def decode_trace(
    chain_id: str, block_hash: str, trace: dict, trace_id: str or None, indent: int
) -> tuple:

    semantics = get_semantics(chain_id, trace["contract_address"], block_hash)

    decoded_trace = dict()
    decoded_trace["trace_id"] = trace_id
    decoded_trace["type"] = (
        "DEPLOY" if trace["entry_point_type"] == "CONSTRUCTOR" else "INVOKE_FUNCTION"
    )
    decoded_trace["entry_point_type"] = trace["entry_point_type"]
    decoded_trace["caller"] = trace["caller_address"]
    decoded_trace["contract"] = trace["contract_address"]

    if decoded_trace["type"] == "INVOKE_FUNCTION":

        decoded_trace["code"] = trace["contract_address"]
        ...
        if not function_abi and "__default__" in semantics["abi"]["functions"]:
            ...
            if function_abi["inputs"][1]["name"] == "calldata_size" or (
                len(trace["calldata"]) > 2
                and int(trace["calldata"][1], 16) != len(trace["calldata"]) - 2
            ):
            ...
            if function_abi["outputs"][0]["name"] == "retdata_size" or (
                len(trace["result"]) > 0
                and int(trace["result"][0], 16) != len(trace["result"]) - 2
            ):
        ...
        if function_abi:
            ...
            decoded_trace["inputs"] = decode_parameters(
                chain_id, trace["calldata"], function_abi["inputs"]
            )
            decoded_trace["outputs"] = decode_parameters(
                chain_id, trace.get("result", []), function_abi["outputs"]
            )
        else:
    ...
    if "events" in trace:
        for event in trace["events"]:
            ...
            decoded_events.append(
                decode_event(chain_id, trace["contract_address"], event, event_abi)
            )
    ...
    for i, sub_trace in enumerate(trace["internal_calls"]):
        ...
        sub_trace_id += str(i)
        sub_calls, sub_events = decode_trace(
            chain_id, block_hash, sub_trace, sub_trace_id, indent + 1
        )
